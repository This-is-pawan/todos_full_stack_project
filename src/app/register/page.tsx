...
const { GetAuthUser,loading,setLoading}=GlobalContext()
  const handleSubmit = async(e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      e.preventDefault()
      const result=await axios.post('/api/auth/register',data,{withCredentials:true})
      if(result.data){
        GetAuthUser()
        route.push('/')
toast.success('register success')
  ...
  return (
    <main className="min-h-screen w-full bg-slate-950 flex items-center justify-center px-5 py-10">
      {/* Background Glow */}
      <div className="absolute top-20 left-1/2 -translate-x-1/2 h-72 w-72 rounded-full bg-cyan-600/20 blur-3xl" />
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6 shadow-2xl backdrop-blur-xl sm:p-8">
          ...
          <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              className="w-full rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-600/20 transition hover:-translate-y-0.5 hover:from-blue-500 hover:to-cyan-400 active:translate-y-0 cursor-pointer flex justify-center"
            >
            {loading?(<AiOutlineLoading  className='animate-spin tansition'/>):'Create Account'}
          ...
          <p className="mt-7 text-center text-sm text-slate-500">
            ...
            <Link
              href="/login"
              className="font-semibold text-blue-400 transition hover:text-cyan-400 cursor-pointer"
            >
        Login      
            </Link>
          </p>
